...
class Snappable extends Component {
  constructor(props) {
    ...
    this._onGestureEvent = event([{nativeEvent: {translationX: dragX, velocityX: dragVX, state: state}}]);
    ...
    //
    const snapPoint = cond(lessThan(add(transX, multiply(TOSS_SEC, dragVX)), 0), -100, 100);

    this._transX = cond(
      eq(state, State.ACTIVE),
      [stopClock(clock), set(transX, add(transX, sub(dragX, prevDragX))), set(prevDragX, dragX), transX],
      [set(prevDragX, 0), set(transX, cond(defined(transX), runSpring(clock, transX, dragVX, snapPoint), 0))],
    );
  }
  render() {
