...
function scaleFriction(value, rest, delta) {
  ...
  const friction = max(1, min(MAX_FRICTION, add(1, multiply(howFar, (MAX_FRICTION - 1) / MAX_VALUE))));
...
function bouncy(value, gestureDiv, gestureActive, lowerBound, upperBound, friction) {
  ...
  return cond(
    ...
    [stopClock(timingClock), stopClock(decayClock), add(value, divide(gestureDiv, friction(outOfBounds)))],
    cond(
