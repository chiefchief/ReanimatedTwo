...
const colors = ['black', 'blue', 'green', 'yellow', 'red', 'gray', 'pink', 'orange'];
...
function ScrollableView({children}: React.PropsWithChildren<Record<never, never>>) {
  const translateY = useSharedValue(0);
  const loverBound = useSharedValue(0);
  const headerHeight = useHeaderHeight();

  function onLayout(evt: LayoutChangeEvent) {
    loverBound.value = windowDimensions.height - headerHeight - evt.nativeEvent.layout.height;
  }
