...
function follow(value) {
  ...
  return block([cond(clockRunning(clock), 0, startClock(clock)), spring(clock, state, config), state.position]);
}
...
class Tracking extends Component {
  constructor(props) {
    ...
    const snapPoint = cond(lessThan(add(transX, multiply(TOSS_SEC, dragVX)), 0), -(width / 2), width / 2);
