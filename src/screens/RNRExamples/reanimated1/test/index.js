...
const {set, cond, startClock, stopClock, clockRunning, block, debug, spring, Value, Clock, proc} = Animated;
...
export default class Example extends Component {
  constructor(props) {
    ...
    this.t = Array.from(Array(40)).map(() => runSpring(new Clock(), Math.random() * -200, Math.random() * 200));
  }
    ...
    return (
      <View style={styles.container}>
        {Array.from(Array(40)).map((_, i) => (
          <Animated.View key={i} style={[styles.box, {transform: [{translateX: this.t[i]}]}]} />
        ))}
      </View>
    );
  }
