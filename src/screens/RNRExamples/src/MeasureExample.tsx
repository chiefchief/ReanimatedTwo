...
function Section({title, children, height, contentHeight, z, show}: React.PropsWithChildren<SectionProps>) {
  ...
  return (
    <Animated.View style={[styles.section, stylez, {zIndex: z}]}>
      ...
      <View>{React.Children.map(children, (element) => React.cloneElement(element as ReactElement, {ref: aref}))}</View>
    </Animated.View>
  );
}
...
function asyncMeasure(animatedRef: RefObject<React.Component>): Promise<MeasuredDimensions> {
...
function SectionHeader({title, animatedRef, contentHeight, show}: SectionHeaderProps) {
