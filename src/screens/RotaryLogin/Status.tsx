...
const Digit = ({passcode, i}: Digit) => {
  ...
  const animatedProps2 = useAnimatedProps(() => ({
    fill: interpolateColor(bin(passcode.value === '0000'), [0, 1], ['#e74c3c', '#2ecc71']),
  ...
  useAnimatedReaction(
    ...
    (l) => {
      if (l === 4) {
        endTransition.value = withTiming(1, {duration: 650, easing: Easing.inOut(Easing.ease)}, () => {
  ...
  return (
    <>
      <Circle cx={origin.x} cy={origin.y} r={r} fill="black" stroke="black" strokeWidth={4} />
      <AnimatedCircle cx={origin.x} cy={origin.y} r={r - 2} fill="white" animatedProps={animatedProps} />
      <AnimatedCircle cx={origin.x} cy={origin.y} r={r - 2} animatedProps={animatedProps2} />
    </>
  );
};
