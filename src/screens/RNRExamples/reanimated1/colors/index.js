...
function colorHSV(h /* 0 - 360 */, s /* 0 - 1 */, v /* 0 - 1 */) {
  ...
  const colorRGB = (r, g, b) =>
    color(round(multiply(255, add(r, m))), round(multiply(255, add(g, m))), round(multiply(255, add(b, m))));
...
export default class Example extends Component {
  ...
  constructor(props) {
    ...
    this._transX = cond(eq(state, State.ACTIVE), add(offsetX, dragX), set(offsetX, add(offsetX, dragX)));

    const offsetY = new Value(200);
    this._transY = cond(eq(state, State.ACTIVE), add(offsetY, dragY), set(offsetY, add(offsetY, dragY)));
