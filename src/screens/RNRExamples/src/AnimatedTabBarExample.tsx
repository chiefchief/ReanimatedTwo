...
const tabs = [
  {name: 'coffee', item: faCoffee},
  {name: 'list', item: faList},
  {name: 'reply', item: faReply},
  {name: 'trash', item: faTrash},
  {name: 'user', item: faUser},
];
...
function Button({item, index, activeIndex, width, position, indicatorPosition}: ButtonProps) {
  const staticIconStyle = useAnimatedStyle(() => {
    const visibility = interpolate(
      indicatorPosition.value,
      [position - width / 2, position - width / 8, position + width / 8, position + width / 2],
      [1, 0, 0, 1],
      Extrapolate.CLAMP,
    );
    return {
...
const Bar: React.FC = () => {
  ...
  const indicatorPosition = useDerivedValue(() => {
    return withTiming(activeIndex.value * tabWidth + tabWidth / 2, {duration: 500});
  });

  const indicatorStyle = useAnimatedStyle(() => ({transform: [{translateX: indicatorPosition.value}]}));

  return (
    <View style={styles.container}>
      <Animated.View style={[{position: 'absolute', left: -tabWidth}, indicatorStyle]}>
        {tabs.map((tab, index) => (
          <ActiveIcon index={index} activeIndex={activeIndex} item={tab.item} width={tabWidth} key={`fg-${index}`} />
        ))}
